...
typedef struct {
    ...
    uint8_t boot_method;      /* 0 = CHS, 1 = EDD, 2 = error */
    uint8_t retry_count;      /* Number of disk read retries */
    ...
    /* Memory size in MB (from INT 15h E820) */
    uint32_t memory_mb;
    ...
    uint8_t reserved[56];  /* Padding for future use */
} bootlog_data_t;
